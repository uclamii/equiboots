...
def test_validate_groups_removes_single_class():
    data = {
        "A": {"y_true": np.array([1, 1, 1])},
        "B": {"y_true": np.array([0, 1, 0])},
    }
    valid = plots._validate_groups(data)
...
def test_eq_plot_residuals_by_group_invalid_group(monkeypatch):
    ...
    monkeypatch.setattr(plt, "show", lambda: None)
    plots.eq_plot_residuals_by_group(data, group="B")  # should print warning, not error
